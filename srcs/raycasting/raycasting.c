...
/*   Updated: 2023/07/09 16:45:46 by susasaki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	initialize_vars(t_info *info)
{
	...
	init_nswe_dirction(info->map.map_data \
		[info->map.y_player][info->map.x_player], &info->vars);
	info->vars.floor_col = create_rgb(info->texture.f_rgb->red,
			info->texture.f_rgb->green, info->texture.f_rgb->blue);
	info->vars.ceil_col = create_rgb(info->texture.c_rgb->red,
			info->texture.c_rgb->green, info->texture.c_rgb->blue);
	info->data.img = exit_mlx_new_image(info->vars.mlx);
	info->data.addr = (unsigned int *)mlx_get_data_addr(info->data.img,
			&info->data.bits_per_pixel, &info->data.line_length,
			&info->data.endian);
	clean_image(&info->data);
	create_xpm_textures(&info->texture, info);
	draw_color_floor_and_ceil(&info->data, info->vars.floor_col,
		info->vars.ceil_col);
...
void	raycasting(t_info *info)
{
	...
	mlx_put_image_to_window(info->vars.mlx, info->vars.win, info->data.img, 0,
		0);
